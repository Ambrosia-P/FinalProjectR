...
```{r}
all_sources <- bind_rows(Combustible_fuels, Hydro, Solar, Wind, Nuclear, Natural_gas, Renewables)

all_sources <- all_sources |> pivot_wider(names_from = "Source", values_from = "Electricity_prod")
write.csv(all_sources, "all_sources.csv", row.names = FALSE)
...
all_sources_list <- split(all_sources, all_sources$Country)

France_prod <- all_sources_list[["France"]]
Germany_prod <- all_sources_list[["Germany"]]
Poland_prod <- all_sources_list[["Poland"]]
Bulgaria_prod <- all_sources_list[["Bulgaria"]]
Finland_prod <- all_sources_list[["Finland"]]
Sweden_prod <- all_sources_list[["Sweden"]]
Denmark_prod <- all_sources_list[["Denmark"]]
Netherlands_prod <- all_sources_list[["Netherlands"]]
Spain_prod <- all_sources_list[["Spain"]]
