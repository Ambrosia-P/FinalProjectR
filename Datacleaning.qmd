...
elec_raw <- read.delim("data/EnergyPricesNewVersion.csv", sep = ";", skip=11)


electricity_prices <- elec_raw[2:10, ]

electricity_prices <- electricity_prices |> select(-matches("^X(\\.|$)")) |> rename(Country = TIME)
colnames(electricity_prices) <- colnames(electricity_prices) |> gsub("^X", "", x = _) |> gsub("\\.", "-", x = _)   


electricity_prices <- electricity_prices |>mutate(across(-Country,~ gsub(",", ".", .) |> as.numeric() * 1000))
...
```{r}
raw <- read.delim("data/Dutch TTF Natural Gas Futures Historical Data.csv", sep= ";")

TTF <- raw |> select(1:2) |> rename(date = 1, price = 2) |> mutate(date  = as.Date(date, format = "%m/%d/%Y"),price = as.numeric(gsub(",", "", price)))

TTF_filtered <- TTF |> filter(date >= as.Date("2019-01-01"),date <= as.Date("2024-12-31")) |> mutate(year = year(date), month_num = month(date), month = factor(month.abb[month_num], levels = month.abb))

table_year_month <- TTF_filtered |> select(year, month, month_num, price) |> arrange(year, month_num) |> select(-month_num) |>pivot_wider(names_from  = month, values_from = price) |> arrange(year)

table_year_month <- table_year_month |> mutate(across(Jan:Dec, as.numeric))

table_year_month_S <- table_year_month |> rowwise() |> mutate(S1 = mean(c_across(Jan:Jun), na.rm = TRUE),S2 = mean(c_across(Jul:Dec), na.rm = TRUE)) |> ungroup()

knitr::kable(table_year_month_S)
...
write.csv2(
  electricity_prices %>% mutate(across(-Country, ~round(., 1))),
  file = "Energypricescleaned.csv",
  row.names = FALSE)

write.csv2(
  table_year_month_S %>% mutate(across(-year, ~round(., 1))),
  file = "Gaspricescleaned.csv",
  row.names = FALSE)
